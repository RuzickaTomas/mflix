...
public class CursorMethodsAndAggEquivalents extends AbstractLesson {
  ...
  public void testFindSortMethod() {
    ...
    assertEquals(Integer.valueOf(999), sortedArray.get(0).getInteger("i"));
  }
  ...
  public void testFindSortAndSkip() {
    ...
    for (Document d : iterableAfterInsert) {
      ...
      assertEquals(Integer.valueOf(10), d.getInteger("i"));
    }
    ...
    assertEquals(skippedFirst.size(), sortedFirst.size());

    // so do all the elements in those lists.
    for (int i = 0; i < 10; i++) {
      assertEquals(skippedFirst.get(i).getInteger("i"), sortedFirst.get(i).getInteger("i"));
    }
  ...
  public void testSortSkipLimit() {
    ...
    for (Document d : cursor) {
      assertEquals(Integer.valueOf(iValue), d.getInteger("i"));
      iValue--;
    }
  }
  ...
  public void testFindLimitAndAggLimitStage() {
    ...
    assertEquals(2, limitedFindList.size());
    ...
    for (Document d : aggLimitCursor) {
      assertNotEquals(-1, limitedFindList.indexOf(d));
    }
  }
  ...
  public void testFindSortandAggSortStage() {
    ...
    assertEquals(findSortList.size(), aggSortList.size());
    for (int j = 0; j < findSortList.size(); j++) {
      assertEquals(findSortList.get(j), aggSortList.get(j));
    }
  }
  ...
  public void testSkipInAggAndFind() {
    ...
    for (Document d : moviesCollection.find(queryFilter).skip(10)) {
      assertEquals(skipAggList.get(index), d);
      index++;
    }
  }
  ...
  public void testPuttingItAllTogether() {
    ...
    assertNotEquals(limitFirstList.size(), findList.size());
    ...
    assertEquals(findList, aggregationList);
  }
