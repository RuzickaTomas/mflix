...
public class UsingPojoLesson extends AbstractLesson {
  ...
  public void testReadUsingPojo() {
    ...
    assertEquals(docActor.getObjectId("_id"), pojoActor.getId());
    assertEquals(docActor.get("name"), pojoActor.getName());
    assertEquals(docActor.get("date_of_birth"), pojoActor.getDateOfBirth());
    assertEquals(docActor.get("awards"), pojoActor.getAwards());
    assertEquals(docActor.get("num_movies"), pojoActor.getNumMovies());
  }
  ...
  public void testMappingDocumentsToActorClass() {
    ...
    // is null.
    assertNotNull(actorDocument, "We should be able to find the actor");
    ...
    // field
    assertEquals(actor.getId(), actorDocument.get("_id").toString());
    assertEquals(actor.getName(), actorDocument.getString("name"));
    assertEquals(actor.getDateOfBirth(), actorDocument.getDate("date_of_birth"));
    assertEquals(actor.getAwards(), actorDocument.get("awards"));
    assertEquals(actor.getNumMovies(), actorDocument.get("num_movies"));
  }
  ...
  public void testReadObjectsWithCustomCodec() {
    ...
    assertEquals(actor1Id.toHexString(), actor.getId());
    ...
    assertEquals(mappedActor.getId(), actor.getId());
    assertEquals(mappedActor.getName(), actor.getName());
    assertEquals(mappedActor.getDateOfBirth(), actor.getDateOfBirth());
    assertEquals(mappedActor.getAwards(), actor.getAwards());
    assertEquals(mappedActor.getNumMovies(), actor.getNumMovies());
  ...
  public void testReadObjectWithCustomFieldCodec() {
    ...
    assertNotNull(stringIdActor);
    // make sure we have an instance of ActorWithStringId
    assertTrue(stringIdActor instanceof ActorWithStringId);
    // the id values match
    assertEquals(actor1Id.toHexString(), stringIdActor.getId());
    ...
    assertNotNull(mappedActor);
    assertEquals(mappedActor.getId(), stringIdActor.getId());
    assertEquals(mappedActor.getName(), stringIdActor.getName());
    assertEquals(mappedActor.getDateOfBirth(), stringIdActor.getDateOfBirth());
    assertEquals(mappedActor.getAwards(), stringIdActor.getAwards());
    assertEquals(mappedActor.getNumMovies(), stringIdActor.getNumMovies());
    // looks like they do, which is great news.
