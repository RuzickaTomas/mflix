...
public class BasicReads extends AbstractLesson {
	 ...
	 * using the Java driver! We've handled setting up our database handle in the
	 * AbstractLesson class, so let's get started.
	 */

	/**
	 * If you've used other drivers in the past, you may be familiar with a findOne
	 * collection method. The Java Driver for MongoDB includes no such method.
	 *
	 * <p>
	 * To find a single document, we'll use the limit method, which you'll learn
	 * about in a later lesson. The following will find the first Document that
	 * matches the query predicate in natural sort order, most likely as they were
	...
	public void testCanFindOne() {
		...
		assertNotEquals(unexpected, actual, "should not be null");

		// we expect the iterator to have nothing left.
		assertFalse(cursor.hasNext(), "the iterator should have no next");
	}

	/**
	 * Let's go ahead and find a document that matches a predicate. For this
	 * example, we'll find a movie where Salma Hayek is listed in the cast field.
	 */
	@Test
	public void testFindOneSalmaHayek() {
		...
		assertEquals(expectedTitle, actual.getString("title"));
		assertEquals(expectedYear, (int) actual.getInteger("year"));
	}

	/**
	 * What might happen if we were to call next() on an iterator that didn't have
	 * anything in it?
	 *
	 * <p>
	...
	/**
	 * To be safe, we should use the tryNext method. This will return null if
	 * nothing exists in the iterator
	 */
	@Test
	public void testTryNext() {
		Document actual = moviesCollection.find(new Document("title", "foobarbizzlebazzle")).iterator().tryNext();
		assertNull(actual);
	}

	/**
	 * Finding one document is useful, but it's only one particular use case. What
	 * if we want to find all documents that match a predicate?
	 *
	 * <p>
	 * One method to do so is below
	 */
	@Test
	...
	/**
	 * Looking at the document, we can see there is a lot of information. What if we
	 * only wanted the title and year? You may be familiar with projection mechanics
	 * in the mongo shell, where we might do something like
	 *
	 * <p>
	 * db.movies.find({cast: "Salma Hayek"}, { title: 1, year: 1 })
	 *
	 * <p>
	 * The Collection object also has projection functionality, but the usage is
	 * different than that of the mongo shell.
	 */
	@Test
	public void testProjection() {
		Document queryFilter = new Document("cast", "Salma Hayek");
		Document result = moviesCollection.find(queryFilter).limit(1)
				.projection(new Document("title", 1).append("year", 1)).iterator().tryNext();
		...
		assertTrue(result.keySet().containsAll(Arrays.asList("_id", "title", "year")));
	}

	/**
	 * As we saw, unless we explicitly remove the _id key it will remain in the
	 * result or results. To remove the _id, we can pass 0 as the projection
	 * argument. Remember, 1 means retain, and 0 means remove.
	 *
	 * <p>
	 * For this example, let's also search for movies where Johnny Depp is a cast
	 * member as well
	 */
	@Test
	public void testProjectsAway_id() {
		...
		Document queryFilter = new Document("cast", new Document("$all", Arrays.asList("Salma Hayek", "Johnny Depp")));

		List<Document> results = new ArrayList<>();
		moviesCollection.find(queryFilter).projection(new Document("title", 1).append("year", 1).append("_id", 0))
				.into(results);
		...
		assertTrue(firstResult.keySet().containsAll(Arrays.asList("title", "year")));
	 ...
	 * Let's Summarize:
	 *
	 * Querying MongoDB through the Driver may feel odd at first, but eventually it
	 ...
	 *
	 * * Finding one document typically involves querying on a unique index, * such
	 * as the _id field.
	 *
	 * * When projecting, by specifying inclusion (for example, title, 1) all *
	 * fields we don't include will be excluded, except for the _id field. If * we
	 * don't want the _id field returned to us we must explicitly exclude * it.
	 *
